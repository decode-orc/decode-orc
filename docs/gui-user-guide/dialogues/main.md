...
* **Draw the preview and scopes on the GPU** uses the graphics hardware for the preview surface and the scope canvases. Leave it on unless drawing is wrong or unstable on your machine — the software path produces the same picture, only more slowly. The change applies to windows opened afterwards, so close and reopen the preview to see it. The box is greyed out when the application was built without GPU support, when the run was started with `orc-gui --no-gpu`, or when the `ORC_GUI_GPU_RENDER` environment variable has already settled the question — in each case something outside the dialog has settled it. `--no-gpu` applies to that run only and leaves whatever is ticked here as you left it. Its tooltip always says which path is actually in use and why, which is what to read when the box is ticked but the drawing is still on the CPU.

    On X11, every start first checks whether this machine's graphics drivers can actually bring an OpenGL context up, in a short throwaway process of its own — the check is done that way because a driver that cannot do it may take the process down with it rather than reporting the failure. A machine that fails the check draws on the CPU for that run, with the tooltip and the **About** box saying so; nothing is remembered, so a machine whose drivers are later repaired simply passes the check next time and goes back to the GPU with nothing to reset. On Linux the GPU path never waits for the display's vertical sync: Qt's swap interval is set to zero and the Mesa and NVIDIA drivers are told the same (`vblank_mode=0`, `__GL_SYNC_TO_VBLANK=0`, each only if not already set in the environment), so a frame is shown as soon as it is drawn rather than held until the next refresh, and a compositor that is slow to take frames cannot stall the application.
