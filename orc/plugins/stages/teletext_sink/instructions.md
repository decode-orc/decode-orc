...
`-` writes the packet stream to the CLI process's standard output instead of a file — CLI only; the GUI rejects this value. Refused together with `write_report` or `export_subtitles`: those write separate files named after `output_path`, which `-` does not identify a location for.
