...
`-` writes the `.tbc` payload to the CLI process's standard output instead of a file (e.g. `orc-cli process disc.orc-project -o "tbc_sink=output_path=-" | orc-cli ...` for chaining) — CLI only; the GUI rejects this value. No `.tbc.db` metadata (SQLite needs to seek, which a pipe cannot do) and no `.pcm`/`.efm` sidecars either — a pipe carries the primary stream alone, matching the TBC Stream Source stage's own read side.
