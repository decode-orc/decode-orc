...
**Include Lead-in/Lead-out** (bool, default off). By default the mapping covers the programme content only: every lead-in and lead-out frame is discarded. Enable this option to keep one frame of lead-in at the head of the mapping and one frame of lead-out at the tail, when the capture contains them. Downstream sinks read the disc's lead-in metadata — the user's code in particular — from that frame, so applications such as LaserDisc emulation need it present. The lead-in frame kept is one that carries the user's code on VBI line 16; where several qualify (or none do) the frame closest to the start of the programme is used. The lead-out frame kept is the first one after the programme. The retained frames are attached after gap detection, so they never register as a break in the picture numbering.

Picture numbers are cross-checked before they are trusted. IEC 60857 carries the CAV picture number twice, on VBI lines 17 and 18, and the tool only trusts a number outright when both lines decoded and agreed. Where a dropout left only one readable line there is no redundancy behind the number, and one flipped bit inside a BCD digit yields a wrong picture number that still decodes cleanly — a `4` arriving as a `6` displaces the picture by 20000. Such a number is therefore checked against the picture numbers of the nearest confirmed frames on either side, and discarded if it does not fit; the disc picture is padded instead, so the other sources of a stack can supply it. The report counts the numbers discarded this way.
...
As a backstop, a gap in the numbering wider than a tenth of the captured frames (never less than 100) is reported rather than padded. No capture is missing that many consecutive pictures, so a gap that wide means a corrupted picture number placed a frame far from the rest of the disc; padding it would bury the real content under thousands of placeholder frames. The frames themselves are still mapped — the tool does not discard captured content on the strength of one suspect number — but the timeline is not continuous across the gap, and the report says so.
