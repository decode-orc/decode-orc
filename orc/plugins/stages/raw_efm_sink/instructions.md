...
`-` writes the t-value stream to the CLI process's standard output instead of a file (e.g. for chaining into another tool's stdin) — CLI only; the GUI rejects this value. The stream is already headerless with no sidecars, so nothing else changes when piping.
