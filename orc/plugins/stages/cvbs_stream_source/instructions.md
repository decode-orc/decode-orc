...
- Every configuration of this stage reports itself streaming-compatible (see `IStreamingCompatibility` in the plugin SDK) — unlike CVBS Source, there is no parameter combination here that isn't safe to pipe, since access is always forward-only within the buffer window regardless of whether `frame_count` is explicit or unbounded.
...
| Yellow | Partially configured. Set `input_path` and `sample_encoding` before triggering (`frame_count` is optional — `0` means unbounded). |
